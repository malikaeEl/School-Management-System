    ...
    <div className={`animate-in fade-in duration-500 w-full flex flex-col gap-8 ${lang === 'ar' ? 'font-arabic' : ''}`}>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div className="flex gap-2 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl">
          {[{ v: 'all', l: 'Tous' }, { v: 'student', l: 'Élèves' }, { v: 'teacher', l: 'Enseignants' }, { v: 'parent', l: 'Parents' }].map(tab => (
            <button
              key={tab.v}
        ...
        <div className="relative">
          <span className="absolute inset-y-0 left-4 flex items-center text-slate-400"><span className="material-symbols-outlined text-sm">search</span></span>
          <input
            type="text"
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Rechercher..."
            className="pl-10 pr-5 py-2.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 dark:text-white text-sm font-medium focus:ring-4 focus:ring-moroccan-green/10 focus:border-moroccan-green outline-none transition-all"
          />
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="p-7 space-y-5 max-h-[75vh] overflow-y-auto">
              ...
              {form.role === 'student' && (
                <div>
                  ...
                  <select value={form.grade} onChange={e => setForm(p => ({ ...p, grade: e.target.value }))} className="w-full px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 dark:text-white text-sm focus:outline-none focus:ring-4 focus:ring-moroccan-green/10 focus:border-moroccan-green transition-all">
