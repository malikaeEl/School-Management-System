    ...
    <div className={`animate-in fade-in duration-500 w-full flex flex-col gap-8 ${lang === 'ar' ? 'font-arabic' : ''}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 uppercase text-[10px] font-black tracking-widest leading-loose">
            Espace Parent — Bienvenue {profile?.firstName}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
           ...
           <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200/50 h-fit">
              <button onClick={() => setActiveSection('academic')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSection === 'academic' ? 'bg-white text-deep-emerald shadow-sm' : 'text-slate-400'}`}>Scolarité</button>
              <button onClick={() => setActiveSection('schedule')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSection === 'schedule' ? 'bg-white text-deep-emerald shadow-sm' : 'text-slate-400'}`}>Emploi du temps</button>
              <button onClick={() => setActiveSection('finance')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSection === 'finance' ? 'bg-white text-deep-emerald shadow-sm' : 'text-slate-400'}`}>Finance</button>
           </div>
      ...
      {!activeChild ? (
        <div className="bg-white p-20 rounded-[2.5rem] border border-dashed border-slate-200 text-center">
          ...
          <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Aucun enfant lié</p>
        </div>
      ) : (
          ...
          {activeChild.library?.overdueBorrows > 0 && (
            <div className="bg-moroccan-red/10 border border-moroccan-red/20 p-6 rounded-[2.5rem] flex items-center justify-between gap-6 mb-8 animate-pulse">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-moroccan-red font-black uppercase tracking-tight">Alerte Bibliothèque : {activeChild.firstName}</h3>
                  <p className="text-moroccan-red/70 text-[10px] font-bold uppercase tracking-widest mt-0.5">
                    {activeChild.firstName} a {activeChild.library.overdueBorrows} livre{activeChild.library.overdueBorrows > 1 ? 's' : ''} en retard.
                  </p>
                </div>
              </div>
              <div className="hidden sm:block px-6 py-2 bg-moroccan-red/20 text-moroccan-red rounded-xl text-[9px] font-black uppercase tracking-widest border border-moroccan-red/10">Action Requise</div>
            </div>
          )}
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm mb-8 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-3 mt-3">
                ...
                <span className="px-4 py-1.5 bg-moroccan-gold/10 text-moroccan-gold rounded-full text-[10px] font-black uppercase tracking-widest border border-moroccan-gold/5">Année 2025-2026</span>
              </div>
            </div>
            <div className="hidden lg:flex items-center border-l border-slate-100 pl-8">
               <div className="text-center">
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Matières</p>
               ...
               <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                  <div className="flex justify-between items-center mb-8">
                     <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
                        ...
                        Historique des Paiements de {activeChild.firstName}
                     </h3>
                  </div>
